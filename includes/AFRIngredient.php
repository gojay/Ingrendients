...
class AFRIngredient
{
    ...
    {
	global $wpdb, $wp_query, $post, $ingredient_settings;
	...
	if ( $wp_query->query_vars["post_type"] == 'ingredient' )
	{
		...
		// set default display, if display is 0
		...
		$template = ( AFR_DEBUG && AFR_PAGE_DEBUG ) ? AFR_PLUGIN_TPL_DIR . '/_debug.php' : $this->_get_template('page', 'ingredient');
		// include the template
		include( $template );
		
		exit();
		
	    } else { // if dont have posts, set 404 error page
	...
	if( $this->substitution->hasSlug() ){
	    ...
	    // get ingredient substitution by slug	    
		...
		// set template for substitutions => templates/calculate-ingredient.php
		...
		if( have_posts() ){
			...
			// set template for substitutions => templates/loop-ingredient.php
	...
	if( $results = $wpdb->get_results( $SQL ) ) {	    
		...
		// set template => templates/loop-ingredient.php
		$template = $this->_get_template('loop', 'ingredient');		
		include( $template );	    
